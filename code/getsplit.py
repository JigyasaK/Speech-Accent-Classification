...
def filter_df(df):
    ...
    english = df[df.native_language == 'english'][:85]
    mandarin = mandarin[mandarin.length_of_english_residence < 10][:85]
    arabic = arabic[arabic.length_of_english_residence < 10][:85]
    # spanish = df[df['native_language'] == 'spanish']
    # spanish = spanish[spanish.length_of_english_residence < 10][:85]
...
def split_people(df,test_size=0.2):
    ...
    '''
    return train_test_split(df['language_num'],df['native_language'],test_size=0.25)
